...
export default function Home() {
  const { activeRole, currentUser, courses, progress, getCourseProgress, blogPosts } = useLMS();
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 min-h-[calc(100vh-5rem)]">
      ...
      <main className="flex-1 space-y-6">
              ...
              <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                  ...
                  <p className="text-xs text-slate-300 mt-1 max-w-2xl leading-relaxed">
                    You are logged in with the <strong className="text-indigo-400">{activeRole}</strong> persona. Use the sticky bar above to toggle roles instantly and test strict access control.
                  </p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="glass-card p-5 rounded-2xl border border-slate-800">
                ...
                <div className="text-2xl font-extrabold text-emerald-400 mt-1">
                  {activeRole === 'Student' ? enrolledCourses.length : 'N/A (Student Role Only)'}
                </div>
        ...
        {/* TAB 2: ADMIN CONTROL PANEL */}
        {activeTab === 'admin' && <AdminDashboard />}

        {/* TAB 3: CONTENT MANAGER / INSTRUCTOR COURSES */}
        {activeTab === 'courses' &&
          (activeRole === 'Student' ? (
            <StudentDashboard activeTab="courses" setActiveTab={setActiveTab} />
          ) : (
            <ContentManagerDashboard />
          ))}

        {/* TAB 4: MY ENROLLED COURSES (STUDENT) */}
        {activeTab === 'my-courses' && (
          <StudentDashboard activeTab="my-courses" setActiveTab={setActiveTab} />
        )}

        {/* TAB 5: STUDENT PROGRESS GRADEBOOK */}
